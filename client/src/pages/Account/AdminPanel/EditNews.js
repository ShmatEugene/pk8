...
const EditNews = () => {
  ...
  const getCurrentPost = React.useCallback(async () => {
    try {
      const fetched = await request(`/api/news/${postId}`, 'GET', null, {
        Authorization: `Bearer ${auth.token}`,
      });
      setCurrentPost(fetched);

      //set thumbnail
      if (fetched.thumbnailLink) {
        const thumbnailImg = {
          title: '',
          file: null,
          link: fetched.thumbnailLink,
        };
  ...
  const onSendClickHandler = async () => {
    try {
      ...
      formData.append('thumbnailLink', thumbnailImg.link);
      ...
      console.log(data);
      setLoading(false);
      history.push(`/admin-panel/edit/news/${data.data.news._id}`);
    } catch (e) {
      setLoading(false);
      console.log(e);
    }
  ...
  const onUpdateClickHandler = async () => {
    try {
      ...
      const data = await axios.put(`${API_URL}api/news/edit/${postId}`, formData, {
        headers: { Authorization: `Bearer ${auth.token}` },
      });
  ...
  const onDeletePostHandler = async (postId) => {
    try {
      ...
      console.log(fetched);
      history.push('/admin-panel/manage-news');
    } catch (e) {
      console.log(e);
    }
  };
      ...
      <div className="account-layout">
        ...
        <div className="account-layout__account-content account-content">
          <div className="account-content__title account-content__title_column">
            ...
            <p className="account-content__subtitle">Раздел Новости</p>
          </div>
          <div className="account-content__actions">
            ...
            {postId && (
              <div className="account-content__post-link">
                <NavLink to={`/news/${postId}`}>Ссылка на страницу</NavLink>
              </div>
            )}
