...
const EditEdu = () => {
  ...
  const getCurrentPost = React.useCallback(async () => {
    try {
      const fetched = await request(`/api/edu/${postId}`, 'GET', null, {
  ...
  const onSendClickHandler = async () => {
    try {
      ...
      const data = await axios.post(`${API_URL}api/edu/add`, formData, {
        headers: { Authorization: `Bearer ${auth.token}` },
      });
      console.log(data);
      setLoading(false);
      history.push(`/admin-panel/edit/edu/${data.data.eduPost._id}`);
    } catch (e) {
      setLoading(false);
      console.log(e);
    }
  ...
  const onUpdateClickHandler = async () => {
    try {
      ...
      const data = await axios.put(`${API_URL}api/edu/edit/${postId}`, formData, {
        headers: { Authorization: `Bearer ${auth.token}` },
      });
  ...
  const onDeletePostHandler = async (postId) => {
    try {
      const fetched = await request(`/api/edu/delete/${postId}`, 'DELETE', null, {
        Authorization: `Bearer ${auth.token}`,
      });
      console.log(fetched);
      history.push('/admin-panel/manage-edu');
    } catch (e) {
      console.log(e);
    }
  };
          ...
          <div className="account-content__actions">
            ...
            {postId && (
              <div className="account-content__post-link">
                <NavLink to={`/edu/${postId}`}>Ссылка на страницу</NavLink>
              </div>
            )}
