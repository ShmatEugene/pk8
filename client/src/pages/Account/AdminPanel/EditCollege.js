...
const EditCollege = () => {
  ...
  const getCurrentPost = React.useCallback(async () => {
    try {
      const fetched = await request(`/api/college/${postId}`, 'GET', null, {
  ...
  const onSendClickHandler = async () => {
    try {
      ...
      const data = await axios.post(`${API_URL}api/college/add`, formData, {
        headers: { Authorization: `Bearer ${auth.token}` },
      });
      console.log(data);
      setLoading(false);
      history.push(`/admin-panel/edit/college/${data.data.collegePost._id}`);
    } catch (e) {
      setLoading(false);
      console.log(e);
    }
  ...
  const onUpdateClickHandler = async () => {
    try {
      ...
      const data = await axios.put(`${API_URL}api/college/edit/${postId}`, formData, {
        headers: { Authorization: `Bearer ${auth.token}` },
      });
  ...
  const onDeletePostHandler = async (postId) => {
    try {
      ...
      console.log(fetched);
      history.push('/admin-panel/manage-college');
    } catch (e) {
      console.log(e);
    }
  };
      ...
      <div className="account-layout">
        ...
        <div className="account-layout__account-content account-content">
          <div className="account-content__title account-content__title_column">
            ...
            <p className="account-content__subtitle">Раздел Колледж</p>
          </div>
          <div className="account-content__actions">
            ...
            {postId && (
              <div className="account-content__post-link">
                <NavLink to={`/college/${postId}`}>Ссылка на страницу</NavLink>
              </div>
            )}
